...
def sendOffer(request,id,offeredid):
    ...
    if (request.method == 'POST'):
        ...
        redirect('posts:exchangePostHome')
...
def search(request):
    ...
    if (request.method == 'POST'):
        ...
        print(f'{keyword}|{catagory}')
        if catagory=='404':
            data = []
            exposts = ExchangePost.objects.filter(setVisible=1)#.filter(title__icontains=keyword)#.filter(title__icontains=keyword).filter(description__icontains=keyword)
            for i in exposts:
                print(i.id)
                data.append((i, ExchangePostImage.objects.filter(post=i.id)[0]))
            print(data)
            return render(request, 'posts/showSearchResult.html', context={'excatagory': excat, 'expost': data})
        else:
            exposts2 = ExchangePost.objects.filter(setVisible=1)#.filter(title__icontains=keyword).filter(catagory__icontains=catagory)#.filter(title__icontains=keyword).filter(description__icontains=keyword).filter(catagory__icontains=catagory)
            data = []
            for i in exposts2:
                print(i.id)
                data.append((i, ExchangePostImage.objects.filter(post=i.id)[0]))
